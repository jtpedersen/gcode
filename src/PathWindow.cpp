...
void PathWindow::setXRotation(int angle) {
...
void PathWindow::setYRotation(int angle) {
  qNormalizeAngle(angle);
  if (angle != m_yRot) {
    m_yRot = angle;
...
void PathWindow::setZRotation(int angle) {
  qNormalizeAngle(angle);
  if (angle != m_zRot) {
    m_zRot = angle;
...
void PathWindow::initializeGL() {
  // In this example the widget's corresponding top-level window can change
  // several times during the widget's lifetime. Whenever this happens, the
  // QOpenPathWindow's associated context is destroyed and a new one is created.
...
void PathWindow::resizeGL(int w, int h) {
  m_proj.setToIdentity();
  m_proj.perspective(45.0f, GLfloat(w) / h, 0.01f, 100.0f);
}

void PathWindow::mousePressEvent(QMouseEvent *event) {
  m_lastPos = event->pos();
}

void PathWindow::mouseMoveEvent(QMouseEvent *event) {
